...
out = foreach ( k = 1:1000, .combine = rbind, .packages =  c('distrEx','mvtnorm') ) %dopar%
{
  ...
  mle1 = EM_algorithm1(runif(1, beta0 - 0.5, beta0+ 0.5), beta1.adjIV, runif(1, gamma - 0.2, gamma + 0.2), log(sigma1+ 0.3), runif(1, log(sigma2 - 0.1), log(sigma2 + 0.2)), runif(1, rho-0.1, rho), 100)
  ...
  write.table(list("Times " = k, "power" = beta1.pval, "res" = c(res2, res1)) , file = "sam1000.power1.gam1rho0.8sig1equ1sig2equ4.txt", append = TRUE, sep = ",", col.names = NA, qmethod = "double")
  return(beta1.pval)
}
